 ...
 *  If you look closely, DP is not needed
 *  We can transform the problem into
 *  Given S and n numbers, represent any number >= S using minimum numbers
 *  solution is obviously greedy
 */
int dp[1001];
struct Shopping {
...
int Shopping::minNumberGreedy(int x,vector<int> v) {
	sort(all(v));
	if(v[0] != 1) return -1;
	int ind = 0, curr = 0;
	int iter = 0;
	while(curr < x) {
		while(ind + 1 < sz(v) && ( (v[ind + 1] - curr == 1) || (curr >= v[ind + 1]))) ++ind;
		curr += v[ind];
		++iter;
	}
	return iter;
